from flask import Flask, request, jsonify, render_template, session, send_file, current_app, redirect, url_for, flash
...
class Proposal(db.Model):
    ...
    revision_number = db.Column(db.Integer, default=1)  # Ensure this line exists
...
class ProposalAmount(db.Model):
    ...
    def __repr__(self):
        return f'<ProposalAmount {self.description}: {self.amount}>'
...
@app.route('/proposal-management', methods=['GET'])
def proposal_management():
    page = request.args.get('page', 1, type=int)
    per_page = 20  # Number of items per page

    # Query bids with pagination
    bids = Bid.query.order_by(Bid.bid_date.desc()).paginate(page=page, per_page=per_page, error_out=False)

    # Query proposals with pagination
    proposals = Proposal.query.order_by(Proposal.created_at.desc()).paginate(page=page, per_page=per_page, error_out=False)

    # Combine bids and proposals for total count
    total_items = bids.total + proposals.total
    total_pages = max(bids.pages, proposals.pages)

    # Fetch combined items for display
    items = bids.items + proposals.items

    return render_template('ProposalManagement.html',
                           bids=bids.items,
...
def save_proposal():
    ...
    app.logger.info(f'Processing proposal with ID: {proposal_id}')

    proposal = Proposal.query.filter_by(bid_id=proposal_id).first()
    if not proposal:
        app.logger.error(f'Proposal not found for ID: {proposal_id}')
    ...
    # Clear existing amounts and options
    app.logger.info('Clearing existing amounts and options')
    ...
    proposal.special_notes = data.get('special_notes', '')
    proposal.terms_conditions = data.get('terms_conditions', '')
...
def create_proposal():
    bid_id = request.args.get('bid_id')
    app.logger.info(f"Attempting to create proposal for bid_id: {bid_id}")
    if bid_id:
        bid = Bid.query.get(bid_id)
        if bid:
            # Pre-populate proposal data from the bid
            proposal_data = {
                'bidID': bid.bid_id,
                'customerName': bid.customer_name,
                'projectName': bid.project_name,
                'bidDate': bid.bid_date.strftime('%Y-%m-%d') if bid.bid_date else '',
                'engineerName': bid.engineer_name,
                'architectName': bid.architect_name,
...
def get_bid_data(bid_id):
    try:
        ...
        bid_data = {
            ...
            'maintenance_labor_rate': float(bid.maintenance_labor_rate) if bid.maintenance_labor_rate is not None else None,
            'local_sales_tax': float(bid.local_sales_tax) if bid.local_sales_tax is not None else None,
            'total_budget': float(bid.total_budget) if bid.total_budget is not None else None,
            'landscaping_total': float(bid.landscaping_total) if bid.landscaping_total is not None else None,
            'drains_total': float(bid.drains_total) if bid.drains_total is not None else None,
            'irrigation_total': float(bid.irrigation_total) if bid.irrigation_total is not None else None,
            'maintenance_total': float(bid.maintenance_total) if bid.maintenance_total is not None else None,
...
# Make sure to call check_and_update_cache() when the app starts
